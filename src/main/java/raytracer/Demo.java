...
public class Demo {
    ...
    private int createQuadProgram() throws IOException {
        ...
        int vshader = DemoUtils.createShader("raytracer/raytracequad.vs", GL_VERTEX_SHADER, "330");
        int fshader = DemoUtils.createShader("raytracer/raytracequad.fs", GL_FRAGMENT_SHADER, "330");
    ...
    private void createComputeProgram() throws IOException {
        int program = glCreateProgram();
        int cshader = DemoUtils.createShader("raytracer/raytracing.glslcs", GL_COMPUTE_SHADER);
        int random = DemoUtils.createShader("raytracer/random.glsl", GL_COMPUTE_SHADER);
        int randomCommon = DemoUtils.createShader("raytracer/randomCommon.glsl", GL_COMPUTE_SHADER, "330");
    ...
    private void present(int fps) {
        ...
        text.drawText(new Vector2d(-0.98f, 0.98f), "FPS: " + fps);
    }

    private void loop() {
        int frameCount = 0;
        long frameTime = System.nanoTime();
        long fpsTime = frameTime;
        int fps = 0;
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            long dt = curTime - frameTime;
            frameTime = curTime;
            frameCount++;

            if (curTime - fpsTime > 1e9) {
                fps = frameCount;
